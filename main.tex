...
本手法では，3.1.1節で検出された動的オブジェクトのマスクとSAM3の検出結果を比較し，重複するオブジェクトを除外する．
...
\begin{enumerate}
\item \textbf{初期学習}: 元の画像データセットを用いてDroneSplatを学習し，動的オブジェクトのマスクを取得する（3.1.1節）．
\item \textbf{オブジェクト検出}: SAM3を用いてすべての車を検出する．
\item \textbf{マスクフィルタリング}: SAM3の検出結果と動的オブジェクトのマスクを比較し，重複するオブジェクトを除外する（3.1.2節）．
\item \textbf{インペインティング}: フィルタリング後の静止オブジェクトマスクに対してLaMaを適用し，不要物を除去した画像を生成する（3.1.3節）．
