...
% XeLaTeXの場合のみフォント設定を読み込む（ローカル環境用）
\ifxetex
  \usepackage{fontspec}   % 欧文フォント操作用
  \usepackage{xeCJK}      % 日本語フォント操作用
  
  % 日本語フォント設定（macOS用、ローカル環境のみ）
  % Overleafではこの設定は使わず、bxjsarticleのデフォルト設定を使用
  \setCJKmainfont{MS PMincho}
  \setCJKsansfont{Hiragino Kaku Gothic ProN}
  \setCJKmonofont{Hiragino Kaku Gothic ProN}
\else
  % Overleaf（pdfLaTeX/LuaLaTeX）の場合
  % bxjsarticleが自動的に適切なフォントを設定します
\fi
